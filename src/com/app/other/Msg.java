package com.app.other;

/**
 * Created at : 4/29/2024
 */
public class Msg {

    public static String Save = "New record has been saved successfully.";
    public static String Save_Failure = "Unable to save record.";
    public static String Update = "Record has been updated successfully.";
    public static String Update_Failure = "Unable to update record.";
    public static String Delete = "Record has been deleted.";
    public static String Loading_failure = "Failed to delete record";
}
